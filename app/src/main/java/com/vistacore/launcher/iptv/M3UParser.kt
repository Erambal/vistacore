...
class M3UParser {
    ...
    // Name/category cleanup lives in [ProviderText] so M3U and Xtream sources
    // strip the same provider noise (US|/24-7 tags, decorations, VIP).
    ...
    private fun parseExtInf(line: String): ExtInfData {
        ...
        return ExtInfData(ProviderText.cleanName(name), logoUrl, ProviderText.cleanCategory(group), tvgId)
    }
